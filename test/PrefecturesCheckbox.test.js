...
describe('PrefecturesCheckbox component', () => {
  it('prefectures', async () => {
    const wrapper = shallowMount(PrefecturesCheckbox, { store, localVue })
    await nextTick()
    expect(wrapper.vm.prefectures).toEqual(prefectures)
    await wrapper.find('#code-1').setChecked()
    expect(wrapper.vm.checkedPrefCode).toEqual([1])
    await wrapper.find('#code-2').setChecked()
    expect(wrapper.vm.checkedPrefCode).toEqual([1, 2])
